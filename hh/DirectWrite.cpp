...
// options：テキストの整形
//=============================================================================
void DirectWrite::DrawString(std::string str, DirectX::SimpleMath::Vector2 pos, D2D1_DRAW_TEXT_OPTIONS options)
{
	// 文字列をワイド文字列に変換
	...
	// フォント設定
	for (size_t i = 0; i < FontNameList.size(); ++i) {
		UINT32 length = FontNameCount[i];
		pTextLayout->SetFontFamilyName(FontList[(int)FontNameList[i]], DWRITE_TEXT_RANGE{ currentIndex, length });
		currentIndex += length;
	}

	// サイズ設定
	currentIndex = 0;
	for (size_t i = 0; i < FontSizeList.size(); ++i) {
		UINT32 length = FontSizeCount[i];
		pTextLayout->SetFontSize(FontSizeList[i], DWRITE_TEXT_RANGE{ currentIndex, length });
		currentIndex += length;
	}

	// 色設定
	currentIndex = 0;
	for (size_t i = 0; i < FontColorList.size(); ++i) {
		UINT32 length = FontColorCount[i];
		ID2D1SolidColorBrush* colorBrush;
		pRT->CreateSolidColorBrush(FontColorList[i], &colorBrush);
		pTextLayout->SetDrawingEffect(colorBrush, DWRITE_TEXT_RANGE{ currentIndex, length });
		currentIndex += length;
	}
...
void DirectWrite::Init()
{
	...
	// バックバッファの取得
	// 型：IDXGISwapChain
	hr = Renderer::GetSwapChain()->GetBuffer(0, IID_PPV_ARGS(&pBackBuffer));



	// dpiの設定
	FLOAT dpiX = (float)GetDpiForWindow(Application::GetWindow());
	FLOAT dpiY = dpiX;
	...
	hr = pD2DFactory->CreateDxgiSurfaceRenderTarget(pBackBuffer, &props, &pRT);
...
void DirectWrite::SetTimeranning(int minutes,int seconds)
{
	// 残り時間を秒単位の文字列に変換
	timeString = "Time:" + std::to_string(minutes) + ":" + std::to_string(seconds);
}

void DirectWrite::SetPosition(DirectX::SimpleMath::Vector2 pos)
{
	position = pos;
}
